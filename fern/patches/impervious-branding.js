...
async function getPathToSourceBranding() {
  ...
  return path.join(root, "brands", "impervious", "branding");
}
...
const brandingPathComponents = ["browser", "branding", "impervious"];
...
async function getTargetPath(pathComponents) {
  return path.join(await getRoot(), "mozilla-release", ...pathComponents);
}
...
async function copy(fromPath, ...to) {
  ...
  return Promise.all(
    to.map(async (toPath) => {
      return fsExtra.copy(from, await getTargetPath(toPath));
    })
  );
}

module.exports = () => ({
  name: "Setup impervious branding",
  paths: [
  ...
  apply: async () => {
    ...
    // white private tab logo
    await copy(
      ["brands", "impervious", "branding", "content", "private-logo-white.svg"],
      [
        "browser",
        "themes",
        "shared",
        "icons",
        "indicator-private-browsing.svg",
      ],
      ["browser", "themes", "shared", "privatebrowsing", "private-browsing.svg"]
    );
    // context-fill private tab logo
    return copy(
      ["brands", "impervious", "branding", "content", "private-logo.svg"],
      ["browser", "themes", "shared", "icons", "privateBrowsing.svg"],
      ["browser", "themes", "shared", "privatebrowsing", "favicon.svg"]
    );
  },
