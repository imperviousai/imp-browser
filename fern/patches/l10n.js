  ...
  try {
    ...
    return JSON.parse(
      await fsExtra.readFile(
        path.join(root, "l10n", `${locale.replace("-", "_")}.json`)
      )
    );
  }
}
...
function replacementLine(key, value, format) {
  if (format === ".ftl") {
    if (Array.isArray(value)) {
      return [`${key} =`, ...value.map((line) => `    ${line}`)].join("\n");
    }
    return `${key} = ${value}`;
  // } else if (format === ".inc") {
...
function patchStrings(replacements, content, format) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (key && replacements[key]) {
      if (format === ".ftl" && !replacements[key].string) {
        ...
        format === ".ftl" &&
        lines[i + 1].trim().startsWith(".label")
      ) {
  ...
  const extraLines = [...keys].map((key) =>
    replacementLine(key, replacements[key].string, format)
  );
